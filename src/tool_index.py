...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "read_file": "Read a file from disk and return its contents. Supports text/code plus Office/EPUB extraction (including .xlsx) and optional offset/limit ranges. Call it after locating a requested file; do not assume an Office file is unreadable without trying.",
    ...
    "glob": "Find FILES by literal name or glob pattern, newest first. When the user names Desktop/Downloads/Documents or another folder, pass that exact path and try the literal filename before a recursive pattern. Prefer over bash find/ls.",
    "ls": "List a directory's entries (folders then files with sizes). Use to see what's in a folder — prefer over bash ls.",
    "get_workspace": "Return the absolute path of the active workspace folder the user is working in. File tools are confined to it; Bash/Python start there and may be OS-sandboxed when sandbox settings are enabled. Call this first when the user refers to 'the project'/'the code'/'this folder' without giving a path, instead of asking them.",
    ...
    "manage_settings": "Change real app settings and tool policies. Use set/get/list/reset for preferences; disable_tool/enable_tool/list_tools for global tool toggles; list_permission_rules/add_permission_rule/delete_permission_rule/clear_permission_rules/list_session_permission_rules/clear_session_permission_rules/permission_metrics for operation-level permissions (allow/deny/ask by bash command, file path, web_fetch domain, or MCP tool); and sandbox_status/set_sandbox for the local OS sandbox. set_sandbox accepts explicit settings or presets: off, local, network_deny, strict_local. Secrets/API keys are read-only.",
