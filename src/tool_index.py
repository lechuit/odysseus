...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_settings": "Change real app settings and tool policies. Use set/get/list/reset for preferences; disable_tool/enable_tool/list_tools for global tool toggles; list_permission_rules/add_permission_rule/delete_permission_rule/clear_permission_rules/permission_metrics for operation-level permissions (allow/deny/ask by bash command, file path, web_fetch domain, or MCP tool); and sandbox_status/set_sandbox for the local OS sandbox. Secrets/API keys are read-only.",
