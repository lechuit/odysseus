...
- Memory is explicit. Use `manage_memory` only when it is available AND the user asks to remember/save/update/forget/list/search memory, or asks what you remember. Do not silently save identity facts, preferences, task details, code discoveries, paths, debug recipes, or temporary project state.
...
- Memory is explicit: use `manage_memory` only when it is available and the user asks to remember/save/update/forget/list/search memory, or asks what you remember.
"""
...
```get_workspace
```
Return the absolute path of the active workspace folder. File tools are CONFINED to it (paths can be RELATIVE to it); Bash/Python start there (cwd) and may be OS-sandboxed when sandbox settings are enabled. Call this first when the user says "the project"/"the code"/"this folder" without a path, instead of asking them. No arguments.""",
...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_memory": "- ```manage_memory``` — Explicit persistent memory. Line 1 = action (list/add/edit/delete/search), rest = content. Use only when the user asks to remember/save/update/forget/list/search memory, or asks what you remember. Do NOT silently save task details, code discoveries, file paths, debug recipes, git history, temporary plans, secrets, or contact details. Recalled memories are hints, not current truth: verify files/functions/flags/dates/project-state claims before relying on them. Contact info for another person belongs in `manage_contact`, not memory.",
    ...
    "manage_settings": "- ```manage_settings``` — View/change REAL app settings, tool toggles, operation permissions, and local sandbox settings. Preferences: `{\"action\":\"set|get|list|reset\",\"key\":\"...\",\"value\":...}`. Tool toggles: `{\"action\":\"disable_tool|enable_tool|list_tools\",\"tool\":\"shell\"}`. Operation permissions: `list_permission_rules`, `add_permission_rule`, `delete_permission_rule`, `clear_permission_rules`, `list_session_permission_rules`, `clear_session_permission_rules`, `permission_metrics`; add rules with `behavior` allow/deny/ask, `tool` (bash/edit_file/web_fetch/mcp__...), `match` (exact/prefix/glob/path/domain/tool), and `pattern`. Sandbox: `sandbox_status` to inspect readiness; `sandbox_self_test` to run controlled runtime enforcement checks; `set_sandbox` with `value` to update `enabled`, `fail_if_unavailable`, `network.deny`, filesystem allow/deny lists, or a preset like `{\"preset\":\"strict_local\"}`.",
    "manage_notes": """\
```manage_notes
...
def _assemble_prompt(tool_names: set, disabled_tools: set = None, compact: bool = False) -> str:
    ...
    if compact:
        ...
        if "manage_memory" in included:
            from src.memory_policy import MEMORY_POLICY_PROMPT
            parts.append("## Memory policy\n" + MEMORY_POLICY_PROMPT)
...
async def stream_agent_loop(
    ...
    if not guide_only and _relevant_tools is not None and not strict_tool_turn:
        ...
        try:
            ...
            if explicit_memory_requested(_retrieval_query):
                _relevant_tools.add("manage_memory")
        except Exception:
            pass
