...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_settings": "- ```manage_settings``` — View/change REAL app settings, tool toggles, operation permissions, and local sandbox settings. Preferences: `{\"action\":\"set|get|list|reset\",\"key\":\"...\",\"value\":...}`. Tool toggles: `{\"action\":\"disable_tool|enable_tool|list_tools\",\"tool\":\"shell\"}`. Operation permissions: `list_permission_rules`, `add_permission_rule`, `delete_permission_rule`, `clear_permission_rules`, `permission_metrics`; add rules with `behavior` allow/deny/ask, `tool` (bash/edit_file/web_fetch/mcp__...), `match` (exact/prefix/glob/path/domain/tool), and `pattern`. Sandbox: `sandbox_status` to inspect readiness; `set_sandbox` with `value` to update `enabled`, `fail_if_unavailable`, `network.deny`, or filesystem allow/deny lists.",
    "manage_notes": """\
```manage_notes
